...
**Recovery** — Primary owner: Daemon Control Plane. Collaborator: Session Runtime. The Daemon Control Plane owns crash-safe state persistence (write-ahead semantics preventing corruption on crash-during-save), progress checkpointing (resuming near the interruption point, not from phase start), circular fix detection (escalating after 3+ occurrences of the same logical error), graceful shutdown (drain mode, grace period, cleanup, lock release), and orphaned work cleanup (periodic scan and termination of work without an active run). The Session Runtime contributes orphaned process cleanup (session processes without active handles). Both services write state safely enough that a crash during save does not corrupt recovery state.
...
- Daemon Control Plane OWNS: recovery (crash-safe persistence, progress checkpointing, circular fix detection), graceful shutdown, work-request-level concurrency, consecutive-failure auto-pause, and orphaned work cleanup.
...
**L1 scenario coverage:** Every L1 scenario in FUNC-AC-SAFETY maps to one of the flows above. Cost Control scenarios (daily budget, per-task cap, execution substrate, subscription-aware management, budget reset) map to the cost control flow and Session Runtime internals. Containment scenarios (isolation, access blocking, content inspection, response offloading, repetition detection, read/write classification, audit, timeout) map to Session Runtime's six containment layers. Concurrency scenarios map to the consecutive failure flow and Daemon Control Plane internals. Rate limiting scenarios map to the rate limit flow. Recovery scenarios (state persistence, progress recovery, circular fix detection, shutdown, orphaned cleanup) map to the graceful shutdown flow and Daemon Control Plane internals. Secrets scenarios map to Session Runtime internals.
