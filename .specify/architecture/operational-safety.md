...
- Daemon Control Plane OWNS an **alert-channel degradation policy (B2)**: a *merge-governed* deployment with **no** configured operator alert channel still boots, but emits a loud startup warning and reports its observability surface as **degraded** (it does not refuse). This realizes FUNC-AC-SAFETY *"rather than continuing silently"* + *"makes its degraded state observable"* without asserting that a channel is *required* — no L1 makes a channel mandatory, so a hard boot-refuse would be net-new Operator policy (deferred as **B2-strict**). A non-governed deployment is unaffected. Independently, whenever an auto-pause or escalation fires while **no** channel is configured, the no-op delivery is made **non-silent** (a structured local warning), so an unattended daemon never escalates into the void.
- Daemon Control Plane OWNS a **truthful liveness surface (B4)**: the always-on observability endpoint reports one of three states so an external monitor can act — **unsafe / no forward progress** (a stuck-run streak at threshold, a detected work-loop stall, a *governed* deployment whose decision transport has failed **at runtime**, a poll in flight past the watchdog idle-timeout while neither paused nor draining, or a **safety** auto-pause), **degraded-but-intentional/transient** (an Operator/manual pause, draining, the startup-degraded window, a governed-without-channel deployment, or a transient alert send failure), and **normal**. The pause cause is carried with the pause so an *intentional* manual pause is never reported as unhealthy, while an *un-tagged* pause defaults to the cautious unsafe interpretation. A non-governed deployment's decision-transport state is never a liveness signal. The transient startup-degraded liveness surface (Recovery domain) hands off to this surface once the Control Plane binds its real endpoint.
- Daemon Control Plane OWNS a **work-loop watchdog (B5, detect-only)**: a periodic check **detects** a wedged work loop using signals the Control Plane already maintains — a run whose durable-progress timestamp has not advanced past an idle-timeout (**run-stall**), or a repo poll that started but never settled past the same idle-timeout (**tick-stall**). On detection it **self-pauses** (a safety pause: claims no new work), notifies the Operator once, and flips the liveness surface to **unsafe**. It explicitly **does NOT** terminate or cancel the live run, and **does NOT** release the held concurrency slot — releasing a slot while the original run is still alive risks a double-release, overlapping work, and a stuck-marked run still mutating shared state. The held slot is recovered by Operator restart (the *terminate* half — actively cancelling a hung run — is deferred as **B5-cancel**). The idle-timeout defaults above the longest bounded in-worker phase so a legitimately-long but *progressing* run never false-positives; it is configurable downward for watched pilots. This realizes the *detect-and-escalate* half of FUNC-AC-SAFETY *"Auto-pause after consecutive failures"* and the observability of *"Task timeout"*.
- Daemon Control Plane OWNS **top-level crash escalation**: an unrecovered fault that escapes normal control flow notifies the Operator (non-silent even with no channel) and exits gracefully (drain + non-zero exit) rather than going dark or hard-exiting. The Operator deployment prerequisite for unattended liveness is an **external monitor that polls the liveness surface** (B4); the out-of-process outbound dead-man's-switch (**B3**) that would cover environments where inbound polling is impossible is deferred as net-new L1.
...
**Work-loop watchdog flow (Daemon Control Plane internal, B5 — detect-only):**
1. A periodic watchdog samples the live work-loop signals: each active run's last durable-progress timestamp and each repo poll's in-flight start time.
2. It computes, against an idle-timeout, whether any active run has made no progress past the timeout (run-stall) or any poll has been in flight past the timeout (tick-stall). A run with no known progress timestamp, or one within the timeout, is not flagged.
3. On a detected stall (and only when not already paused/draining): set the paused flag with a *safety* pause cause, record the stall for the liveness surface, and notify the Operator exactly once (the self-pause makes subsequent ticks no-ops, which is the notify-once mechanism).
4. The watchdog does **not** decrement the active-run count and does **not** cancel the run. The held concurrency slot persists until Operator restart.
5. An Operator resume clears the pause and the recorded stall; the held slot, if any, is recovered by restart.

**Truthful liveness-reporting flow (Daemon Control Plane internal, B4):**
1. An external monitor queries the always-on liveness surface.
2. The Control Plane gathers a snapshot of its live state (pause + pause cause, draining, consecutive-stuck count, watchdog stall, governed decision-transport runtime health, the startup-degraded flag, the alert-channel-degraded flag, and recent alert-send success).
3. A pure evaluation maps the snapshot onto unsafe / degraded / normal per the boundary policy above, and the surface answers with the corresponding status so the monitor can page, alert, or stay quiet.
