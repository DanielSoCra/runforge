...
- Verifier Gate IS CONSULTED BY: the Daemon Control Plane's pipeline state machine, before any autonomous outcome is sought for a lane and ahead of every other gate. The precondition is realized at two seams with the same evaluator: before autonomous **implementation dispatch** (the execution seam — a withheld verdict blocks the implement dispatch itself and halts the run to the Operator, doing no throwaway work) and before the **merge** disposition (ahead of the Lane Engine's assign/evaluate). A verifier-less governed lane is withheld at both.
