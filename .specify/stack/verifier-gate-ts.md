...
**Composed ahead of the lane engine, never inside it.** The control-plane wiring calls `evaluateVerifierGate` *after* `assignLane` resolves the run's lane (so the assigned lane's `verifier` declaration is known) and *before* `evaluateMergeEligibility` or any autonomous disposition; only a `'verifier-gated'` result proceeds to the lane-engine eligibility path, after which compliance + earned-autonomy still apply on top. This keeps the lane-engine's pure functions ignorant of the verifier precondition — the gate is a sibling module, exactly like the scope tripwire is a step inside `evaluateMergeEligibility` rather than a config flag.
...
- Run the gate *after* `assignLane` (the assigned lane's `verifier` must be known) but *before* `evaluateMergeEligibility` or any autonomous disposition — never after eligibility. If it runs after eligibility, a momentary code path could act on a positive eligibility result before the precondition is checked; placing it between lane assignment and eligibility keeps the lane-engine result correctly moot when autonomy is withheld.
