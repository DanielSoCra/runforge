...
- `test_paths` references the parser tests (`src/knowledge/parse-commits.test.ts`) — there is no direct unit test for a prompt template. The parser tests exercise the commit format defined here: after changing the format in `worker.md`, run those tests to verify the new format is still parseable.
