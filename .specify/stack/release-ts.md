...
2. **Release Ledger** — a new append-only per-deployment Postgres store following the `@runforge/decision-index` writer/read-model shape (see Key Decisions), fronted by a per-deployment manager exactly like `DecisionEscalationManager`.
...
- **Debut authorization = a THIRD OPTION on the first production-release APPROVAL (FUNC-AC-FLEET debut gate; consumed by STACK-AC-EARN-IN).** The release lane is where the Operator witnesses a deployment's debut into pre-approved unattended merging — folded into the release approval it already reserves, never a new beat. The decision protocol's option answer carries a single `chosen_option` and rejects a simultaneous structured `answer` (`decision-request.ts`), so the debut is encoded as an extra OPTION, not a side-flag: while the deployment has **no prior approved release** (derived: its Release Ledger holds no prior `decision` event whose `chosen_option` is `approve` or `approve-with-debut`), `buildReleaseDecisionRequest` offers `options: [{id:'reject'},{id:'approve'},{id:'approve-with-debut'}]` with `answer_schema:{kind:'option'}`; on every subsequent release it offers only `[{id:'reject'},{id:'approve'}]`. The binding is to the first *approval*, not the first proposal — a *rejected* first release still offers `approve-with-debut` on the next one, and a rejected decision can never carry the debut through. The decision event records its `chosen_option` in `detail_json`. `hasDebutAuthorization(deployment)` returns true **iff a `decision` event carries `chosen_option === 'approve-with-debut'`** — a plain `approve` (or any `reject`) never authorizes the debut. Both `approve` and `approve-with-debut` proceed with the release identically (the executor treats the release the same); the only difference is whether the debut is witnessed. The release lane only *offers, records, and serves* the option; the debut gate that consumes it (and the "is this the first unattended merge" derivation from registry autonomy history) lives in STACK-AC-EARN-IN. A deployment with no production-release path never reaches this builder, so `hasDebutAuthorization` stays false and earn-in's debut fails closed to a per-event Operator decision — the intended no-release-path behavior.
