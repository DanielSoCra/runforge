...
- **Spend attribution on `cost_events` + narrow spend readers** — `cost_events` carries two nullable attribution columns added by forward-only migration `0002`: `provider` (`text` — the model-provider name the Session Runtime routed the session to, per FUNC-AC-RUNTIME-ADAPTERS) and `usage_units` (`bigint` — the total token count the runtime reported for the session, when the adapter reports one). `NULL` means *unattributed*: rows that predate the migration, or sessions whose adapter cannot report the value, stay `NULL` — consumers (STACK-AC-SPEND-OBSERVABILITY) surface them as an explicit `unattributed` bucket and never invent a value. `recordCostEvent` takes an optional trailing `attribution` argument (`{ provider?, usageUnits? }`) so existing call sites stay valid. Three narrow, read-only readers are the contract the spend projection consumes: `CostEventStore.listForWindow(window)` (events with `recordedAt` in `[from, to)`), `RunStore.attributionFor(runIds)` (per-run `{ runId, projectId, completedAt }` — the run→project/completion-time join), and `RepoStore.namesFor(projectIds)` (`{ id, name }` display names, `owner/name`). Chosen over exposing the pooled `db` or the full Store classes: the readers keep the projection's read-only authority visible in its signatures and trivially fakeable in unit tests.
...
```ts
// cost_events spend attribution — nullable on purpose: NULL = unattributed, never invented
