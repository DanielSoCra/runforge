...
>
> **Spec history (v2.3, 2026-06-24, earn-in floor ruling):** Hardens pre-approved earn-in so the safety claim rests on a **non-configurable mechanism, not on the Operator's act of declaring a bar**. The Operator's ruling: a typed bar proves a human touched the value, not that the value is safe, so **pre-approved auto-promotion is fail-closed (disabled) by default** and may take effect only when the lane's promotion policy clears a set of non-configurable earn-in floors — a minimum required amount and recency of clean track record, exclusion of any lane that recorded a red event within that window, the scope verification holding, the lane being reversible on a later red, and the lane being verifier-gated — *and* the Operator's declared bar sits at or above every one of those floors. The bar remains required (a missing bar still fails closed), but it is now **necessary, not sufficient**: an explicit-but-weak bar that sits below a floor also fails closed, exactly as a missing one does. The floors are platform mechanism that no profile, lane, or configuration may lower; a bar may only make promotion harder than the floor, never easier. Starting a deployment's first regulated release deliberately conservative remains an option that deployment may choose in its profile — it is not a universal requirement of this spec. This is a content amendment under an already-approved spec; `status` stays `approved`.

## Problem Statement

The platform finishes many changes on its own, and each one meets a single decision before it joins the deployment's shared mainline: may it proceed on the platform's judgement, or must it wait for the Operator? Getting this wrong in one direction starves the Operator of leverage — the platform produces work but never relieves the Operator, which defeats its purpose. Getting it wrong in the other lets a change that deserved a human's eyes slide in unattended.
...
**Scenario: A lane with a pre-approved earn-in policy auto-widens only when it clears the earn-in floors**
- Given a deployment's profile declares a **pre-approved earn-in policy** for a lane — the Operator's grant given in advance — and the lane is verifier-gated, its risk level is one eligible for an autonomous proceed, the scope verification is holding, the lane is reversible on a later red, and the Operator's declared bar sits at or above every non-configurable earn-in floor
- When the lane's recorded track record meets that bar — having satisfied the floor's minimum amount and recency of clean record and recorded no red event within that window
- Then the lane's autonomy widens without a per-promotion decision, the auto-promotion is recorded with the policy, the floors it cleared, and the track record that triggered it, and it remains reversible fleet-wide

**Scenario: Pre-approved auto-promotion is disabled until the earn-in floors are cleared**
- Given a deployment has declared a pre-approved earn-in policy for a lane, but the lane's promotion policy does not clear a non-configurable earn-in floor — its required clean track record is too small or too stale, a red event was recorded within the floor's window, the scope verification is not holding, the lane is not reversible on a later red or not verifier-gated, or the Operator's declared bar sits below a floor
- When the lane's recorded track record would otherwise meet the declared bar
- Then the lane does not auto-widen — the pre-approval is inert, the lane keeps its current more cautious treatment, and any widening still requires an explicit, per-event Operator decision; a declared bar that sits below a floor fails closed exactly as an undeclared bar does
...
- Every deployment begins with all risk levels held for the Operator and widens only by the Operator's grant — given per promotion, or in advance as a pre-approved earn-in policy — trust is earned, never granted at the outset, for levels and for lanes alike
- Pre-approved auto-promotion never takes effect on the strength of a declared bar alone: it is disabled unless the lane's promotion policy clears every non-configurable earn-in floor — a minimum amount and recency of clean track record, exclusion of any lane with a red event in that window, the scope verification holding, the lane reversible on a later red, and the lane verifier-gated — and the Operator's declared bar sits at or above each floor; a missing bar and an explicit-but-weak bar that sits below a floor both fail closed
- Once a level's or lane's autonomy is earned, routine changes of that class stop reaching the Operator, measurably relieving the Operator of that work
...
- The safety of pre-approved auto-promotion rests on **non-configurable earn-in floors, not on the act of declaring a bar**. Pre-approved auto-promotion is **fail-closed (disabled) by default** and takes effect only when the lane's promotion policy clears every floor — a minimum amount and recency of clean track record, exclusion of any lane that recorded a red event within that window, the scope verification holding, the lane reversible on a later red, and the lane verifier-gated — **and** the Operator's declared bar sits at or above each floor. A declared bar is **necessary but not sufficient**: a missing bar and an explicit bar that sits below any floor both fail closed identically. The floors are platform mechanism; no profile, lane, or configuration may lower or waive them, and a declared bar may only make promotion harder than a floor, never easier. Choosing to start a deployment's first regulated release deliberately conservative is a per-deployment option held in its profile, not a universal requirement of this spec
