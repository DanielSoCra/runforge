...
- Commands: `pnpm --filter @auto-claude/dashboard test|lint|typecheck|e2e`; daemon equivalents. Root `pnpm -r test` does NOT run e2e.
...
2. Baselines BEFORE changes: `pnpm --filter @auto-claude/dashboard test` and `pnpm --filter @auto-claude/daemon test` tails recorded; after: no new failures; both packages' lint + typecheck green; `pnpm --filter @auto-claude/dashboard e2e` green locally.
...
```
pnpm --filter @auto-claude/dashboard test <dashboard gate paths> && pnpm --filter @auto-claude/daemon test <daemon gate path> && pnpm --filter @auto-claude/dashboard typecheck && pnpm --filter @auto-claude/daemon typecheck
```

(E2E gate runs via `pnpm --filter @auto-claude/dashboard e2e` — implementer runs it locally as DoD; CI enforces it post-Task-2.)
