...
**Files:** `auto-claude.config.json:13-16`; read-side sanity in `packages/daemon/src/config.ts:250-255` (schema, defaults `staging:'staging'`) — schema change NOT needed (values are free-form strings).
...
4. Guard test (gate encodes this, G3): a test reading `auto-claude.config.json` asserting `branches.staging`/`branches.production` are not retired branches (`dev`) — cheap, honest, prevents regression. Place near existing config tests (grep `auto-claude.config.json` under `packages/daemon/src` for the loader/validation test home).
...
1. `traceability.yml`: add any new files to the owning node's `code_paths`, new test files to `test_paths` (STACK-AC-CONTAINMENT for session-runtime work; STACK-AC-CONTROL-PLANE for daemon/server work). Run `pnpm --filter @auto-claude/daemon test src/infra/traceability-paths.test.ts`.
2. **Capture the failure baseline BEFORE changes** (`pnpm --filter @auto-claude/daemon test 2>&1 | tail -30` — real-PG suites skip without a local Postgres; note which failures pre-exist), then after all tasks: full `pnpm --filter @auto-claude/daemon test` shows **no new failures vs. that baseline**, plus `pnpm --filter @auto-claude/daemon typecheck` and `pnpm --filter @auto-claude/daemon lint` fully green.
...
```
pnpm --filter @auto-claude/daemon test <gate test paths> && pnpm --filter @auto-claude/daemon typecheck
```
