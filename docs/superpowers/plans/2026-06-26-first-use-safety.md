...
- Daemon tests: `pnpm --filter @auto-claude/daemon exec vitest run <path>`. Decision-index tests: `pnpm --filter @auto-claude/decision-index exec vitest run <path>`.
- Real-Postgres-gated tests run only when `AUTO_CLAUDE_TEST_DATABASE_URL` is set (CI sets it); locally they `skipIf(!REAL_PG)`.
- Gate checks before any commit: `pnpm --filter @auto-claude/daemon lint && pnpm typecheck`. Repo lint: `pnpm lint`.
...
**Test:** `pnpm --filter @auto-claude/daemon exec vitest run src/control-plane/decision-escalation/manager.test.ts`
...
**Test:** `pnpm --filter @auto-claude/daemon exec vitest run src/control-plane/merge-decision-wiring.integration.test.ts` + the daemon resume tests.
...
- else if `!decisionManager.isAvailable()` → `return err(<reason>)` with an operator-readable message distinguishing: index **disabled** (`!isEnabled()` → "set AUTO_CLAUDE_DECISION_INDEX_ENABLED=1"), **enabled-but-unreachable** (`isEnabled() && !isAvailable()` → "decision index unreachable: <reason>").
...
**Test:** `pnpm --filter @auto-claude/daemon exec vitest run src/control-plane/daemon.test.ts -t "boot guard"`
...
**Test:** `pnpm --filter @auto-claude/daemon exec vitest run src/control-plane/decision-escalation/__fixtures__/fake-decision-ledger.test.ts`
...
**Acceptance:** the new test passes CI-default (no `AUTO_CLAUDE_TEST_DATABASE_URL`).
**Test:** `pnpm --filter @auto-claude/daemon exec vitest run src/control-plane/daemon.test.ts -t "round-trip"`
...
**Acceptance:** with `AUTO_CLAUDE_TEST_DATABASE_URL` set, the extended test reaches+asserts the merge; without it, it skips (unchanged).
**Test (local, with PG):** `AUTO_CLAUDE_TEST_DATABASE_URL=<url> pnpm --filter @auto-claude/daemon exec vitest run src/control-plane/daemon.test.ts -t "integrate park resume"`
...
**Acceptance:** `pnpm --filter @auto-claude/daemon exec vitest run src/infra/traceability-paths.test.ts` passes (all `code_paths` resolve).

**PR1 gate (all must pass):** `pnpm install --frozen-lockfile && pnpm --filter @auto-claude/daemon lint && pnpm typecheck && pnpm --filter @auto-claude/daemon exec vitest run src/control-plane/decision-escalation src/control-plane/daemon.test.ts src/control-plane/merge-decision-wiring.integration.test.ts src/infra/traceability-paths.test.ts`
...
**Test:** `pnpm --filter @auto-claude/daemon exec vitest run src/config.test.ts` (+ a daemon pauseReason test).
...
**Test:** `pnpm --filter @auto-claude/daemon exec vitest run src/control-plane/notify.test.ts` + a pause-site test.
...
**Test:** `pnpm --filter @auto-claude/daemon exec vitest run src/control-plane/daemon.test.ts -t "alert channel"`
...
**Test:** `pnpm --filter @auto-claude/daemon exec vitest run src/control-plane/repo-manager.test.ts`
...
**Test:** `pnpm --filter @auto-claude/daemon exec vitest run src/control-plane/daemon.test.ts -t "active run progress"`
...
**Test:** `pnpm --filter @auto-claude/daemon exec vitest run src/control-plane/daemon.test.ts -t "watchdog"`
...
**Test:** `pnpm --filter @auto-claude/daemon exec vitest run src/control-plane/server.test.ts -t "health"`
...
**Test:** `pnpm --filter @auto-claude/daemon exec vitest run src/control-plane/daemon.test.ts -t "process handlers"` (a focused unit around the handler fn).
...
**Acceptance:** `pnpm --filter @auto-claude/daemon exec vitest run src/infra/traceability-paths.test.ts` passes.

**PR2 gate:** `pnpm install --frozen-lockfile && pnpm --filter @auto-claude/daemon lint && pnpm typecheck && pnpm --filter @auto-claude/daemon exec vitest run src/control-plane src/config.test.ts src/main.test.ts src/infra/traceability-paths.test.ts`
...
Per the spec's "End-to-end verification design" against the demo deployment (`~/code/auto-claude-demo-runtime`, Postgres `autoclaude_demo`, target `cause-driven-tasks`). Captured in `docs/superpowers/plans/2026-06-26-first-use-safety.execution-log.md`.
