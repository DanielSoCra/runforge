...
3. **Extract `protected-store.ts` + redaction into a new, not-registered-by-default `@auto-claude/sanitizer-redaction` package.** Contains: encrypted redaction store, `protected://` ref helpers, `protected_refs` writer/reveal, a withholding Sanitizer (implements the `@auto-claude/sanitization` port), + its crypto/reveal tests. Core ingest never calls it by default.
