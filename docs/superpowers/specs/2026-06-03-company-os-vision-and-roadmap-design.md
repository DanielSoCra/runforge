...
Visual overview: the cockpit consumer's `docs/integration/2026-06-03-company-os-vision-overview.html`
Cockpit prototype: the cockpit consumer's `docs/integration/2026-06-03-cockpit-redesign-prototype.html`

## 1. Context & problem
Six overlapping projects (runforge, the cockpit consumer, the ops-workflow project, the goal-loop project, plus an OSS agent-shell project & Archon) are secretly **one system sliced differently** — each reinvents agents/roles, goals, workflows, gates, specs. Months of work, no convergence. This spec converges them into **one product** and a phased roadmap, starting with a sharp first wedge.

Existing reality to reconcile: the Operator already runs a **separate agent-shell-based org for the regulated pilot's billing/ops side** (team CEO+CPO+Engineer; roadmap CLA-10, board-approved; engine M0–M4 done, only live regulated-domain send missing). The company OS must *integrate with / grow from* that, not ignore it (see §8 integration point).
...
| Wedge | First proof | **Depth-first on the regulated pilot** (the hardest real case) |
...
| Spine | Foundation | **(a) Own it — copy the agent-shell project's shape, add the ops-workflow project + runforge** |
| Dropped | ~~sensitive data~~ | **Out of scope** — customer data lives on customers' own servers; runforge never touches it |

## 3. Architecture — the backbone
**Org → Department → Goal → Work.**
- **Org** = tenant (the regulated pilot, tenant-a, an agency) with an **org chart** of **Departments** (teams of agent roles + humans).
- **Goal** = the steering primitive a human sets (multiple, concurrent, prioritized).
- **Work** = the unit a department executes. **The unifying trick:** for **engineering**, a Work item *is* the full runforge L0→L3 pipeline (gates + deterministic resume). For other departments, a Work item is a lighter **workflow** (the ops-workflow project's recipes / Archon). Same abstraction, variable depth → SWE depth stays first-class; others stay light.
...
- **the cockpit consumer** → the shared-room cockpit / front-end (keep).
- **the ops-workflow project** → the ops + workflow layer for non-eng departments (keep).
- **the goal-loop project** → the goal-loop / goals primitive (keep).
- **the agent-shell project** → the shell we copy & own: multi-org/tenant, org chart, agents-as-employees, approvals, adapters, multiplayer auth, embedded Postgres.
...
## 5. Roadmap (phases)
- **Phase 0 — the wedge (prove the depth):** engineering department, on the regulated pilot, autonomously ships real features end-to-end, steered by goals, gated through the v2 cockpit, multiplayer-light. ≈ what we HAVE + cockpit redesign + the goal primitive + reliability.
- **Phase 1 — the shell:** the agent-shell-shaped spine (Orgs/Departments/org-chart/tenancy/identity/multiplayer); engineering = the first deep department.
- **Phase 2 — widen:** non-eng departments via the ops-workflow layer; onboard tenant-a + the agency; multi-goal steering.
...
- **Slice 1 — Reliable end-to-end engine** *(this cycle)*: the daemon ships a feature **L0→L3→merged** on a real repo, reliably, fully tested. Prereq: **consolidate** the validated, unmerged work (PRs #709 workspace, #710 gate1, #711 l2-designer paths, the cockpit consumer #7 answerable inbox) via `/deep-review` + merge. Fix the live blocker **#49 (approved l2-gate re-parks / duplicate handler livelock)**. Make the gap-8 git-credentials fix permanent (#43). E2E + smoke for the full loop.
- **Slice 2 — The goal primitive:** "set a goal → it creates the L0/L1 + the work item → the pipeline runs." Generalize the goal-loop project's goal-loop.
- **Slice 3 — The cockpit v2 (real):** implement the approved prototype into the cockpit consumer via `/frontend-design`, wired to live data (`/api/runs/live` + the index), Playwright-tested.
- **Slice 4 — Multiplayer-light:** identity, assignment/claim, threads, presence.
- **Slice 5 — Wire to the regulated pilot + full e2e/smoke + reconcile with the separate agent-shell-based regulated-pilot org.**
...
Scope:
1. **Consolidate:** `/deep-review` then merge #709, #710, #711, the cockpit consumer #7 (resolve conflicts). One green main per repo.
...
- **Reconcile with the existing agent-shell-based regulated-pilot org** (ops/billing already runs there): is Phase 1's shell a fork we own, or do we extend the running agent-shell project and own it later? Decide before Phase 1.
