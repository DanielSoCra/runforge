...
- Run: `pnpm --filter @auto-claude/daemon exec vitest run src/control-plane/decision-api.test.ts src/control-plane/daemon.test.ts src/operator-learning src/infra/traceability-paths.test.ts` + lint + typecheck.
