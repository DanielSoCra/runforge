...
async function toSourceWritten(t: TempDb, f: ReturnType<typeof makeOutboxWithGeneration>, id: string) {
...
describe("FINDING 1 — periodic reconcile recovers a prior-gen lease-expired `executing` row (no restart)", () => {
  ...
  function insertExecutingRequeue(decisionId: string, effId: string, runId: string, gen: string) {
    t.db
      .insert(outboxTable)
      .values({
        id: effId,
  ...
  it("THE STRAND: pendingEffectDecisions() (the old sweep) does NOT see a prior-gen executing requeue row, so a sweep that only drives pending strands it", async () => {
    ...
    insertExecutingRequeue(id, requeueId, "run-1", "gen-prior-dead");

    // The drive-pending sweep enumerator misses it (only `reserved` rows count).
    expect(f.outbox.pendingEffectDecisions().find((p) => p.decision_id === id)).toBeUndefined();
  ...
  it("NEVER touch a CURRENT-generation executing row: a periodic reconcile tick leaves a live in-flight requeue alone (round-2 guarantee)", async () => {
    ...
    insertExecutingRequeue(id, requeueId, "run-1", gen);
    ...
    const ob = t.db.select().from(outboxTable).where(eq(outboxTable.id, requeueId)).all()[0]!;
