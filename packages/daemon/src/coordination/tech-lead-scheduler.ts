...
import type { CycleTrigger, SignalDigest, TechnicalProposal, TriageDecision, TriageApplyResult, UntriagedIssue } from './tech-lead/schemas.js';
...
export interface TechLeadSchedulerConfig {
  ...
  triageDailyCap: number;
}

export interface TechLeadSchedulerDeps {
  assembleDigest: (trigger: CycleTrigger, config: TechLeadSchedulerConfig, triageContext: { untriagedIssues: UntriagedIssue[]; remainingCap: number }) => Promise<SignalDigest>;
  ...
  fetchUntriagedIssues: (cap: number) => Promise<UntriagedIssue[]>;
  getTriageRemainingCap: () => Promise<number>;
  applyTriageDecisions: (decisions: TriageDecision[], remainingCap: number) => Promise<TriageApplyResult>;
}
...
export function createTechLeadScheduler(
  ...
  async function runCycle(trigger: CycleTrigger): Promise<void> {
    ...
    try {
      ...
      // 2. Prepare triage inbox (cap-bounded before session)
      const remainingCap = await deps.getTriageRemainingCap();
      const untriagedIssues = await deps.fetchUntriagedIssues(remainingCap);

      // 3. Assemble signal digest
      const digest = await deps.assembleDigest(trigger, config, { untriagedIssues, remainingCap });

      // 4. Spawn Tech Lead session with digest
      ...
      // 6. Store proposals (if any)
      if (parsed.data.proposals.length > 0) {
        await deps.storeProposals(parsed.data.proposals);
      }

      // 7. Apply triage decisions (if any)
      if (parsed.data.triageDecisions.length > 0) {
        await deps.applyTriageDecisions(parsed.data.triageDecisions, remainingCap);
      }

      // 8. Route protocol triggers (if any)
