...
describe('mapDocumentToRecord', () => {
  ...
  it('maps document fields to RecordMarker fields', () => {
    const result = mapDocumentToRecord(baseDoc, 'technical_pitfall', 'vault:mistakes', 'my-vault');
    expect(result.marker.artifactPatterns).toEqual(['src/session-runtime/**/*.ts']);
    expect(result.marker.description).toBe('Always reset the flag in a finally block.');
    ...
    expect(result.sourceId).toContain('20-Areas/Engineering/Mistakes/my-note.md');
  });

  it('uses L3 default confidence of 0.5 when document confidence is 0', () => {
    const doc = { ...baseDoc, confidence: 0 };
    const result = mapDocumentToRecord(doc, 'technical_pitfall', 'vault:mistakes', 'my-vault');
