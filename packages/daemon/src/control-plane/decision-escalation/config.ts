/**
 * Decision-escalation env reader — the ONE place the `RUNFORGE_DECISION_*`
 ...
 *
 *   - RUNFORGE_DECISION_INDEX_ENABLED   (default false; truthy = 1/true/yes)
 *   - RUNFORGE_DECISION_INDEX_PATH      (default `decision-index.sqlite`,
 *       resolved ABSOLUTE against stateDir)
 *   - RUNFORGE_DECISION_PROTECTED_DIR   (default `decision-protected/`,
 *       resolved ABSOLUTE against stateDir)
 *   - RUNFORGE_DECISION_PROTECTED_KEY   (base64 AES-256; if unset, generate 32
...
function loadOrGenerateKey(stateDir: string): string {
  const explicit = process.env.RUNFORGE_DECISION_PROTECTED_KEY;
...
export function readDecisionIndexConfig(stateDir: string): DecisionIndexEnvConfig {
  ...
  const enabled = envEnabled(process.env.RUNFORGE_DECISION_INDEX_ENABLED);
  ...
  const protectedDir = resolveAgainst(
    stateDir,
    process.env.RUNFORGE_DECISION_PROTECTED_DIR ?? DEFAULT_PROTECTED_DIR,
  );
 ...
 * unless the operator acknowledges the cutover (RUNFORGE_DECISION_INDEX_CUTOVER_ACK).
 *
...
function assertLegacyStoreCutoverAcked(stateDir: string): void {
  const acked = envEnabled(process.env.RUNFORGE_DECISION_INDEX_CUTOVER_ACK);
  if (acked) return;
  const legacyPath = resolveAgainst(
    stateDir,
    process.env.RUNFORGE_DECISION_INDEX_PATH ?? DEFAULT_LEGACY_SQLITE_PATH,
  );
  ...
  throw new Error(
    `decision-index: a legacy sqlite decision store exists at ${legacyPath}; it may hold ` +
      ...
      `export tool to salvage rows, OR set RUNFORGE_DECISION_INDEX_CUTOVER_ACK=1 to proceed ` +
