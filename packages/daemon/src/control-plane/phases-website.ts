...
const PHASE_DELIVERABLES: Partial<Record<Phase, string[]>> = {
  intelligence: ['docs/01-analysis/raw_firecrawl_data.json', 'docs/01-analysis/intelligence_report.md'],
  brand:        ['docs/02-brand/brand_guide.md', 'docs/02-brand/brand_assets.json', 'docs/02-brand/tailwind.config.mjs'],
...
export function createWebsitePhaseHandlers(
  ...
  const withCheckpointGate = (currentPhase: Phase) => async (_run: RunState): Promise<PhaseEvent> => {
    ...
    const comment = formatCheckpointComment(String(currentPhase), deliverables);

    await octokit.issues.createComment({ owner, repo, issue_number: issueNumber, body: comment }).catch(console.error);
    await octokit.issues.addLabels({ owner, repo, issue_number: issueNumber, labels: ['checkpoint-paused'] }).catch(console.error);
    await octokit.issues.removeLabel({ owner, repo, issue_number: issueNumber, name: 'in-progress' }).catch(console.error);
