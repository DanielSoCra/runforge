...
1. **Read the spec chain.** L3 spec → L2 spec → L1 spec → the relevant L0 (`.specify/L0-vision.md` for concierge, `.specify/L0-ac-vision.md` for runforge — pick whichever L0 lists the L1 as a child in `.specify/traceability.yml`). Higher layers win on contradictions (per CLAUDE.md).
